...
Each Cursor conversation gets its own tmux server on a dedicated socket (`tmux -L cursor-<proj>-<id>`), and since v3.1 both the socket and every task session name carry the project name as a prefix — so parallel sessions, including directors running in different repos, never collide with each other or with your own tmux. Every non-trivial task runs as one tmux session hosting a local CLI agent, babysat by a cheap Composer 2.5 subagent that starts it, watches it, injects mid-run instructions, and reports back. The main Cursor agent only coordinates, reviews, and synthesizes.
...
- **Claude Code CLI handles complex work:** Fable with High Effort, or Opus 5 with Extra High (the `opus` alias tracks the newest Opus). In the Claude Code variant the same CLI doubles as a third worker lane on Opus 5, quota-gated because it shares the director's own pool.
...
- **Quota-aware routing, machine-wide:** Claude via `claude -p "/usage"`; Codex `/status` and Grok `/usage show` — previously interactive-only — are driven in throwaway tmux sessions with `send-keys` + `capture-pane`. Results land in a machine-level `~/.director/quota.json` shared by every director on the host, refreshed at session start and whenever an entry is older than 30 minutes. Unexposed limits stay UNKNOWN; across all three pools, whichever has the most weekly quota remaining gets the work, and an agent is sidelined once a known limit reaches 80% or its 5-hour window is spent.
...
| Complex reasoning, architecture, or debugging | Claude Code CLI: Fable · High Effort, or Opus 5 · Extra High |
| Routine or moderately complex implementation | Codex CLI: GPT 5.6 SOL · Medium Effort, Grok CLI: grok-build via ACP (TUI lane falls back to Grok's default model), or Claude Code CLI — whichever pool has the most weekly quota left |
...
## v3.1 (2026-07-28) — cross-project isolation, Opus 5 worker lane, machine-level quota

- **Project-prefixed control plane.** A `PROJ` prefix (the main repo's directory name, sanitized) is baked into the tmux socket (`ccdir-<proj>-<id>` / `cursor-<proj>-<id>`) and every task session name (`<proj>-t<N>-<agent>-<slug>`). Parallel directors in different repos used to mint identical `t<N>-…` names and interfere with each other; now a bare name is a bug, and orphan-socket triage distinguishes "another repo's director" (never touch) from "this repo's dead predecessor" (reconcile, then ask).
- **Opus 5 joins the worker pool.** The Claude Code variant gains a third worker: a separate `claude` CLI on `--model opus` (the alias tracks the newest Opus — Opus 5 today), dispatchable on Lane A/B/C, resumable via `claude --resume`, cross-reviewing with Codex and Grok — so a quota-dead reviewer now has an alternative. Because this lane drains the same account pool the director coordinates from, it only takes implementation while Claude's pool is clearly the roomiest and under ~60% used.
- **Quota state is machine-level.** Per-repo `.tasks/quota.json` moves to `~/.director/quota.json`, shared by every director on the host: one entry per agent holding the raw probe line, its interpretation (5-hour + weekly, % left), and a timestamp. Entries older than 30 minutes are refreshed at session start and before long dispatches; a parallel director's fresh probe is reused instead of re-probed. Remaining weekly quota across all three pools is the routing key — the roomiest eligible agent gets the work, and a spent 5-hour window sidelines an agent until reset.

## v3.2 (2026-08-05) — numbering leases, collision-proof artifacts, worker role immunity

Driven by a field audit of a production control plane: three documented task-id collisions between parallel directors in the SAME repo (v3.1 only isolated sockets and session names across repos — the id space and `t<N>.*` artifact names stayed repo-global). The worst incident false-completed a watcher via a stale `.done` and overwrote another line's report.

- **Numbering leases.** Lock-free `max(id)+1` is retired. A director now scans `.tasks/` for ACTUAL files (briefs/reports/.done — TASKS.md alone lies when a parallel line hasn't landed its rows), then records a 30-id lease line (`lease: t<A>–t<A+29> socket=…`) above the table before using any number. Foreign leases are untouchable; dispatch re-probes `ls .tasks/t<N>*` for stragglers. Letter suffixes are legalized: r / f / e\<n\> derive from the parent id, live inside its lease, and don't count toward max.
- **Collision-proof artifacts.** The brief's closing protocol string now refuses to overwrite an existing `t<N>-report.md` (it writes `t<N>-report-2.md` and flags the collision). Exactly one TASKS.md row per task — status edits happen in place; "addendum" duplicate rows had corrupted machine scans and the max computation.
- **Worker role immunity.** Every brief must OPEN with a verbatim IMPLEMENTER identity guard, repeated in the dispatch command line: headless workers twice loaded the host's director bootstrap line and declared themselves director — one hijacked the ledger and raced the real director's tmux sessions, another burned a whole cycle trying to dispatch its own workers inside a sandbox; a repo-level auto-commit rule also overrode a brief's "do not commit". The guard overrides CLAUDE.md / AGENTS.md / PROTOCOL.md, bans touching the ledger and dispatching, and makes the brief win over repo-level instructions; the auto-loaded bootstrap file carries a matching EXCEPTION escape-hatch line (merged with the parallel fix pushed as dc04afd).
- **RECONCILE watches for neighbors.** The ritual also scans t* files and lease lines — fresh artifacts outside your lease mean a parallel director is live; its lease and tasks are respected, never claimed or cleaned.
