...
(prod cutover, data migration, destructive infra) where dispatch adds risk instead of
removing it — declare it first, log why in JOURNAL.md (get my OK if I'm around), and
verify each step before the next. If a CLI flag is rejected, run `<cli> --help` before
dispatch.
...
- Bulk mechanical waves (mass renames, translations, screenshot pipelines) → a cheap
  ...
  narrow — otherwise rework ping-pong costs more than sol would have.
- Trivial one-liners → foreground.
...
- TASKS.md — one row per task or request, machine-scannable:
  ...
  dropped(why) / taken-over. origin = short quote of who asked ("user: 看看隔壁
...
- HANDOFF.md — (re)write at wrap-up, when your own budget hits ≥80%, when I mention
  switching accounts, or when your own output degrades (malformed tool calls):
  mission, in-flight tasks + exact resume commands, next 3 actions, gotchas. A fresh
...
Steer with short send-keys nudges; longer → write .tasks/t<N>-steer.md and send 'Read
.tasks/t3-steer.md and adjust.' Completion = the worker's .done touch.
...
idle minutes (ERROR=idle-timeout). Crash SOP: annotate the brief with what already
completed (prevents duplicate commits on redispatch), redispatch on Lane B, log the
incident in JOURNAL.md. Codex stays on Lane A: codex-acp 0.16.0 bundles a core that
rejects gpt-5.6-sol; after a `brew upgrade codex-acp` retry with:
...
Watching — zero model tokens while waiting. Liveness = .done sentinel + LOG GROWTH;
pane-hash is a secondary signal only (it false-alarmed 12+ times on long compiles /
buffered output and never caught a real hang). .tasks/bin/watch.sh:
  #!/bin/bash
  ...
  exit 1
Run one per task via Bash run_in_background; you are woken on exit — RECONCILE, then:
  0 → read t<N>-report.md + diff, go to Pairing (report missing = abnormal exit: check
      log/pane first).
  3 → startup-dead: capture the pane once. Typical causes: send-keys swallowed by a
      shell-prompt hijack (brew/omz update banners), CLI startup crash, a broken
      ~/.codex/hooks.json. Relaunch once with the same brief; a second 3 → bring me
      the capture. (Startup hangs once burned 45–70 min per incident under timeouts.)
  2 → capture once and judge yourself (you're already awake): DONE (sentinel forgotten
      — verify report) / WORKING (restart watch) / STUCK or WAITING (steer on Lane B,
      or bring me the capture). Session gone = judge the same way.
  1 → timeout: capture evidence. If the log tail shows a quota/limit error, schedule a
      timed redispatch at the reset time (field-proven recovery). Else extend once /
      steer / kill+redispatch. Update TASKS.md either way.

Non-blocking discipline (what keeps you always-available):
- After dispatching: update TASKS.md, END the turn. Never wait, poll, or digest in the
  foreground — watchers wake you.
- Any foreground command expected to exceed ~60s (builds, test suites, long probes)
  runs in a tmux session or background shell with its own sentinel instead. The
  foreground is for dispatch, judgment, and answering me.
- Dispatch before you digest: a new request mid-review is routed FIRST (Intake row →
  steer a running Lane B / queue .tasks/t<N>-followup.md for a Lane A / write+dispatch
  a new brief), then resume. My follow-ups rarely name task ids — resolve by topic
  against TASKS.md; genuinely ambiguous between two tasks → ask me one short question.

Subagents — narrow, cheap, read-only (replaces the old blanket ban):
- ALLOWED on a cheap model (Sonnet-class or below): one-shot pane judgment; digesting
  a multi-MB task log or report into findings; read-only repo/system scouting; a
  TASKS.md reconcile sweep. Self-contained prompt, MUST return a report — an
  unreported watch/scout is a failed job. Verify a subagent's conclusions against
  primary evidence before repeating or acting on them (one unverified relay has
  already misinformed the user).
- FORBIDDEN: writing code, debugging, long-form analysis, substituting for Pairing.
...
Takeover: if I attach and start driving a session (input you didn't send, or I say
